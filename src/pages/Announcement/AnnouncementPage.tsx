...
const AnnouncementPage = () => {
  ...
  const { handleFileChange, imageUrl } = useUploadFormHooks();
  const { postAnnouncements } = useServerAnnouncement();
  const [announcement, setAnnouncement] =
    useState<Announcement>(AnnouncementEmpty);
  ...
  const formHandler = async (e: FormEvent) => {
    e.preventDefault();
    if (validateForm() && imageUrl) {
      await postAnnouncements({
        ...announcement,
        img: imageUrl,
        location: loc,
        region: `${townName?.address.state}`,
        town: `${townName?.address.town}`,
        city: `${townName?.address.city}`,
        village:`${townName?.address.village}`,
      });
      navigation(-1);
    }
  };
